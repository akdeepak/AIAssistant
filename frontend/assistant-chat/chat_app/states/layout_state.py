...
class LayoutState(rx.State):
    ...
    async def submit_assistant(self, files: list[rx.UploadFile]):
        ...
        # Step 2: Upload, chunk, and index knowledge base
        if files:
            first = files[0]
            try:
                file_name = getattr(first, "name", None) or getattr(
                ...
                # Run the blocking API call in a background thread so the
                # progress bar can advance through the stages while it works.
                ...
                # Animate through Upload → Chunking → Indexing while
                # the backend processes everything in a single call.
                progress_stages = [
                    (25, "Uploading files…",        0.8),
                    (30, "Uploading files…",        0.6),
                    (40, "Chunking documents…",     0.8),
                    (50, "Chunking documents…",     1.0),
                    (60, "Indexing documents…",     1.0),
                    (70, "Indexing documents…",     1.5),
                    (75, "Indexing documents…",     2.0),
                ]

                for target, step_text, delay in progress_stages:
                    if upload_task.done():
                        break
                    self.creation_progress = target
                    self.creation_step = step_text
                    yield
                    await asyncio.sleep(delay)

                # If the backend is still working, hold at 75 %
                ...
                self.creation_progress = 80
                self.creation_step = "Finalizing…"
                yield
            except Exception:
                self.creating_assistant = False
                self.assistant_created = False
